"""
Script para eliminar retroactivamente gastos con origin='inventory'.
...
def fix_inventory_expenses():
    """Elimina todos los gastos con origin='inventory'"""
    ...
    with app.app_context():
        try:
            ...
            confirm = input(f"\n⚠️  ¿Eliminar {count_before} gastos de inventario? (sí/no): ")
            
            if confirm.lower() not in ['sí', 'si', 's', 'yes', 'y']:
                print("❌ Operación cancelada")
                return
            
            # Eliminar gastos de inventario
