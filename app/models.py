...
def _ensure_business_settings_columns() -> None:
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            if 'habilitar_doble_turno_arqueo' not in cols:
                conn.execute(
                    text(
                            'ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS habilitar_doble_turno_arqueo BOOLEAN NOT NULL DEFAULT FALSE'
                    )
                )
            if 'arqueo_turno_1_desde' not in cols:
                conn.execute(
                    text(
                            "ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS arqueo_turno_1_desde VARCHAR(5) NULL DEFAULT '08:00'"
                    )
                )
            if 'arqueo_turno_1_hasta' not in cols:
                conn.execute(
                    text(
                            "ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS arqueo_turno_1_hasta VARCHAR(5) NULL DEFAULT '16:00'"
                    )
                )
            if 'arqueo_turno_2_desde' not in cols:
                conn.execute(
                    text(
                            "ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS arqueo_turno_2_desde VARCHAR(5) NULL DEFAULT '16:00'"
                    )
                )
            if 'arqueo_turno_2_hasta' not in cols:
                conn.execute(
                    text(
                            "ALTER TABLE business_settings ADD COLUMN IF NOT EXISTS arqueo_turno_2_hasta VARCHAR(5) NULL DEFAULT '08:00'"
                    )
                )
...
class BusinessSettings(db.Model):
    ...
    habilitar_doble_turno_arqueo = db.Column(db.Boolean, nullable=False, default=False)

    arqueo_turno_1_desde = db.Column(db.String(5), nullable=True, default='08:00')

    arqueo_turno_1_hasta = db.Column(db.String(5), nullable=True, default='16:00')

    arqueo_turno_2_desde = db.Column(db.String(5), nullable=True, default='16:00')

    arqueo_turno_2_hasta = db.Column(db.String(5), nullable=True, default='08:00')
...
class SalePayment(db.Model):
    ...
    sale = db.relationship('Sale', backref='payments', lazy=True)

    __table_args__ = (
        db.Index('ix_sale_payment_company_sale', 'company_id', 'sale_id'),
    )
...
class CashCount(db.Model):
    ...
    shift_code = db.Column(db.String(16), nullable=True, index=True)
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint('company_id', 'count_date', 'shift_code', name='uq_cash_count_company_date_shift'),
